...
`slovnyk` у Piton задається літералом через `{}` з рядковими ключами, працює з доступом через `m["key"]`, підтримує оновлення значень і вкладені структури на кшталт списків та інших словників. Також `dovzhyna()` повертає кількість полів у словнику.
...
Скрипт показує створення профілю користувача, читання окремих полів, оновлення ключів, додавання нового поля та зміну вкладеного значення в `nalashtuvannya`.
...
* [ ] **⬇️ Установшщик** Додати NSIS установщик для `Windows`, щоб `piton` автоматично встановлювався на систему. Додати його в AUR і зробити .deb і AppImage пакети
