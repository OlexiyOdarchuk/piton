...
Piton не тільки виконує ваш код, але й вміє автоматично його креслити\! Рушій візуалізації працює на **[rombik](https://rombik.ishawyha.dev)** — власній бібліотеці автора для побудови блок-схем за **ДСТУ** (чистий Go, без зовнішніх залежностей). В одну команду ви перетворюєте ваші скрипти на професійні векторні блок-схеми.

Це працює не просто як "малювалка тексту". Інтерпретатор Piton створює повноцінне "Супер-Дерево" (Super AST), з режимом -all може автоматично підтягнути всі залежності через `vykorystaty`, будує окрему схему на кожну функцію і підписує їх іменем модуля. Головний потік програми виділяється як "Головна програма" для легкого читання.
...
### 🌐 WebAssembly (для веб-плейграунду)

Для лендингу [ishawyha.dev](https://ishawyha.dev) Piton збирається у дві окремі WASM-цілі:

| Артефакт              | Тулчейн  | Розмір (raw)  | Розмір (brotli) | Що містить                      |
| --------------------- | -------- | ------------- | --------------- | ------------------------------- |
| `piton-runner.wasm`   | TinyGo   | ~220 KB       | ~80 KB          | лише інтерпретатор (`runPiton`) |
| `piton-viz.wasm`      | Go       | ~3.6 MB       | ~683 KB         | ДСТУ-візуалізатор (rombik)      |

Runner вантажиться при першому кліку «Запустити»; viz — лазі-вантажиться лише коли користувач тисне «Блок-схема». Розщеплення зменшує bandwidth для більшості відвідувачів на ~90%. (Перехід на rombik зробив viz майже у 8 разів меншим: було ~5.5 МБ brotli з рушієм D2.)

Зібрати обидві цілі:

```bash
nix develop --command bash scripts/build-wasm.sh ./dist
```

Скрипт також копіює відповідні `wasm_exec.js` glue-файли (TinyGo має свій, Go — свій) і прокручує бінарі через `wasm-opt`.
...
- **TinyGo + binaryen** (для збірки WASM-плейграунду через `scripts/build-wasm.sh`)
...
- [x] **🪧 HashMap** Реалізація стуктури даних "Хешмапа" (`slovnyk`), щоб оперувати даними як з map[] в Go або dict в Python
- [x] **📃 Документація:** Написати повноцінну документацію для мови, аби її можна було зрозуміти без перегляду коду
- [x] **❄️ Flake for Nix:** Додати flake.nix для користувачів nix та NixOS, щоб зручно встановлювати Piton працювати в проєкті через nix develop
